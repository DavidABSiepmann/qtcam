...
public:
	...
	bool open(const std::string &device, bool useWrapper = true);
	void close();
	int read(unsigned char *p, int size);
	int ioctl(unsigned cmd, void *arg);
	bool ioctl(const std::string &descr, unsigned cmd, void *arg);
	...
	static std::string pixfmt2s(unsigned pixelformat);
