...
public:
    ...
    //Q_INVOKABLE void setCameraName(const std::string msg);
    //std::string getCameraName();

    //Modified by Nithyesh
    /*
     * Removed arg std::vector<std::string> from function as it was unused.
     * Previous fn signature was
     * int uvccamera::findEconDevice(std::vector<std::string>, std::string)
     */
    int findEconDevice(std::string);
    int enumerateDevices(std::vector<std::string>*, std::vector<std::string>*);
    void getDeviceNodeName(std::string);
    bool closeAscellaDevice();

    static libusb_device_handle* handle;  /* handle for USB device */
    int kernelDriverDetached;

    static std::multimap<std::string, std::string> cameraMap;
    static std::multimap<std::string, std::string> serialNumberMap;
    static int hid_fd;
    static std::string hidNode;
    /**
...
private:
     ...
     */
    std::multimap<std::string, std::string> pidVidMap;
    /**
     * @brief cameraEnumMap - This map contains Vid,pid value and corresponding enum value
     */
    std::multimap<std::string, CommonEnums::ECameraNames> cameraEnumMap;
...
public:
  ...
  void serialNumberToCheckUsbSpeed(std::string serialnumber);
     ...
     * @return
     */
    bool initExtensionUnit(std::string cameraName);
     ...
     */
    void currentlySelectedDevice(std::string deviceName);
...
//      */
//     bool getTorchState(uint8_t *torchState);
...
//      */
//     bool getFlashState(uint8_t *flashState);
...
// signals:
//     void deviceStatus(std::string title, std::string message);
// public slots:
