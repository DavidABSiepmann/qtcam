...
bool See3CAM_24CUG::getEffectMode(unsigned char &out)
{
    // hid validation
...
bool See3CAM_24CUG::getDenoiseValue(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getAutoExpROIModeAndWindowSize(unsigned char &out1, unsigned char &out2)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
    ...
    if (uvc.sendHidCmd(g_out_packet_buf, g_in_packet_buf, BUFFER_LENGTH))
    {
        if (g_in_packet_buf[6] == GET_FAIL)
        {
            return false;
                 ...
                 g_in_packet_buf[6] == GET_SUCCESS)
        {
            out1 = g_in_packet_buf[2];
            out2 = g_in_packet_buf[5];
            return true;
        }
    }
    return false;
...
bool See3CAM_24CUG::setROIAutoExposure(See3CAM_24CUG::camROIAutoExpMode see3camAutoexpROIMode, uint vidResolnWidth, uint vidResolnHeight, uint xCord, uint yCord, std::string winSize)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
    ...
    if (see3camAutoexpROIMode == AutoExpManual)
    {
        ...
        g_out_packet_buf[6] = std::stoi(winSize); // window size
    }
...
bool See3CAM_24CUG::getQFactor(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getOrientation(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getFaceDetectMode(unsigned char &out1, unsigned char &out2, unsigned char &out3)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
    ...
    if (uvc.sendHidCmd(g_out_packet_buf, g_in_packet_buf, BUFFER_LENGTH))
    {
        ...
        {
            out1 = g_in_packet_buf[2];
            out2 = g_in_packet_buf[3];
            out3 = g_in_packet_buf[4];
            return true;
        }
    }
    return false;
...
bool See3CAM_24CUG::getFrameRateCtrlValue(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getExposureCompensation(unsigned int &exposureCompValue)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
    ...
    if (uvc.sendHidCmd(g_out_packet_buf, g_in_packet_buf, BUFFER_LENGTH))
    {
        if (g_in_packet_buf[6] == GET_FAIL)
        {
            return false;
                 ...
                 g_in_packet_buf[6] == GET_SUCCESS)
        {
            exposureCompValue = (((u_int8_t)g_in_packet_buf[2]) << 24) | (((u_int8_t)g_in_packet_buf[3]) << 16) | (((u_int8_t)g_in_packet_buf[4]) << 8) | (((u_int8_t)g_in_packet_buf[5]) << 0);
            return true;
        }
    }
    return false;
...
bool See3CAM_24CUG::setExposureCompensation(unsigned int exposureCompValue)
{
    ...
    {
        std::cerr << "Given exposure compensation value is invalid" << std::endl;
        return false;
    }
    ...
    if (uvc.sendHidCmd(g_out_packet_buf, g_in_packet_buf, BUFFER_LENGTH))
    {
        if (g_in_packet_buf[6] == SET_FAIL)
        {
            std::cerr << "Failed to set exposure compensation value" << std::endl;
            return false;
        }
        else if (g_in_packet_buf[0] == CAMERA_CONTROL_24CUG &&
...
bool See3CAM_24CUG::getBurstLength(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::setBurstLength(uint burstLength, uint &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getSmileDetectMode(unsigned char &out1, unsigned char &out2)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getFlickerDetection(unsigned char &out)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
...
bool See3CAM_24CUG::getFlashState(unsigned char &out)
{
    if (uvccamera::hid_fd < 0)
    {
        return false;
...
bool See3CAM_24CUG::getStreamMode(uint &streamMode, bool &autoFunctionLock)
{
    // hid validation
    if (uvccamera::hid_fd < 0)
    {
