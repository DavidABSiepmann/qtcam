...
bool v4l2::open(const std::string &device, bool useWrapper)
{

	m_device = device;
	m_useWrapper = useWrapper;
    m_fd = ::open(device.data(), O_RDWR | O_NONBLOCK);
...
bool v4l2::ioctl(const std::string &descr, unsigned cmd, void *arg)
{
	clear();
...
void v4l2::error(const std::string &error)
{
	if (!error.empty())
        fprintf(stderr, "%s\n", error.data());
}

std::string v4l2::pixfmt2s(unsigned id)
{
	std::string pixfmt;
